...
def make_grayscale(input_image):
	imgarray = np.fromstring(input_image, dtype='uint8')
	print(imgarray)
	img_d = cv2.imdecode(imgarray, cv2.IMREAD_UNCHANGED)
	print(img_d)

	img_g = cv2.cvtColor(img_d, cv2.COLOR_RGB2GRAY)
	status, output_image = cv2.imencode('.png', img_g)
	print(status)
	return output_image
